...
def _read_local_bytes(file_path: str) -> bytes:
    ...
    allowed_roots = [Path(base).expanduser().resolve() for base in settings.LOCAL_FILE_INGEST_BASE_PATHS]
    if not any(root == target or root in target.parents for root in allowed_roots):
        raise HTTPException(status_code=403, detail="file_path is outside allowed ingest paths")

    if not target.is_file():
        raise HTTPException(status_code=404, detail="file_path does not exist or is not a file")
