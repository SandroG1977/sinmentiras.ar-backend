...
def _parse_cached_answer(cached_answer: str) -> dict:
    try:
        ...
        return parsed if isinstance(parsed, dict) else _make_fallback_result(cached_answer)
    except (json.JSONDecodeError, ValueError):
        return _make_fallback_result(cached_answer)


def _invoke_structured_result(prompt: str) -> dict:
    ...
    if isinstance(structured, dict):
        return structured
    return _make_fallback_result(result.get("output", ""))
...
def run_agent(prompt: str) -> tuple[dict, str]:
    ...
    best_cached = _find_best_cached(query_chunks)

    if best_cached:
        cached_answer_str, cached_model, _similarity = best_cached
        return _parse_cached_answer(cached_answer_str), f"cache:{cached_model}"

    structured = _invoke_structured_result(prompt)
