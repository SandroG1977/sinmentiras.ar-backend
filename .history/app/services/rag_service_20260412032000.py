...
class LocalRAGService:
    ...
    def _load_chunks(self) -> None:
        with self._lock:
            ...
            self._vectors = self._load_embeddings_with_cache(chunks)
    ...
    def _load_embeddings_with_cache(self, chunks: list[RAGChunk]) -> list[list[float]]:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            cached = rag_store.get_cached_embedding(text_hash)
        ...
        if texts_to_embed:
            ...
            for text_idx, chunk_idx in enumerate(text_indices):
                embedding = new_embeddings[text_idx]
                vectors[chunk_idx] = embedding
                # Cache the new embedding
                text_hash = hashlib.sha256(chunks[chunk_idx].text.encode()).hexdigest()
                rag_store.cache_embedding(text_hash, embedding)
    ...
    def _extract_html_title(self, raw_html: str, fallback: str) -> str:
        match = re.search(r"<title>(.*?)</title>", raw_html, re.IGNORECASE | re.DOTALL)
        if not match:
            return fallback
        return html.unescape(re.sub(r"\s+", " ", match.group(1))).strip() or fallback

    def _html_to_text(self, raw_html: str) -> str:
        text = re.sub(r"(?is)<(script|style).*?>.*?</\1>", " ", raw_html)
        text = re.sub(r"(?i)<br\s*/?>", "\n", text)
        text = re.sub(
            r"(?i)</p>|</div>|</h1>|</h2>|</h3>|</h4>|</h5>|</li>", "\n", text
        )
    ...
    def _fetch_infoleg_law(self, law_id: int) -> tuple[str, dict[str, object]]:
        ...
        text = self._html_to_text(texact_response.text)
        title_text = self._extract_html_title(texact_response.text, f"Ley {law_id}")
